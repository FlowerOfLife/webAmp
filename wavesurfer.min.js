 ...
 var WaveSurfer = {
 	...
 	init: function (t) {
 		this.params = WaveSurfer.util.extend({}, this.defaultParams, t), this.markers = {}, this.once("marked", this.bindMarks.bind(this)), this.savedVolume = 0, this.isMuted = !1, this.loopSelection = this.params.loopSelection, this.minPxPerSec = this.params.minPxPerSec, this.createBackend(), this.createDrawer(), this.on("loaded", this.loadBuffer.bind(this))
 	},
 	createDrawer: function () {
 		...
 		this.drawer = Object.create(WaveSurfer.Drawer[this.params.renderer]), this.drawer.init(this.params), this.drawer.on("redraw", function () {
 			...
 			t.drawer.progress(e)
 		}), this.drawer.on("mousedown", function (e) {
 			t.seekTo(e)
 		}), this.params.dragSelection && (this.drawer.on("drag", function (e) {
 			t.selMark0 && t.selMark0.percentage != e.startPercentage && t.seekTo(e.startPercentage), t.updateSelection(e)
 	...
 	drawBuffer: function () {
 		...
 		else t = Math.round(this.getDuration() * this.minPxPerSec);
 		this.drawer.drawPeaks(this.backend.getPeaks(t), t), this.drawer.progress(this.backend.getPlayedPercents()), this.redrawMarks(), this.fireEvent("redraw")
 	},
 	...
 	empty: function () {
 		this.clearMarks(), this.backend.loadEmpty(), this.drawer.drawPeaks({
 	...
 	updateSelection: function (t) {
 		var e = this,
 			i = t.startPercentage,
 			r = t.endPercentage,
 			s = this.params.selectionColor;
 		if (i > r) {
 			var n = i;
 			i = r, r = n
 		}
 		this.selMark0 ? this.selMark0.update({
 			percentage: i
 		}) : this.selMark0 = this.mark({
 			id: "selMark0",
 			percentage: i,
 			color: s
 		}), this.drawer.addMark(this.selMark0), this.selMark1 ? this.selMark1.update({
 			percentage: r
 		}) : (this.selMark1 = this.mark({
 			id: "selMark1",
 			percentage: r,
 			color: s
 		}), this.selMark1.loopEnd = !0, this.selMark1.on("reached", function () {
 	...
 	getSelection: function () {
 		...
 		var t = this.getDuration(),
 			e = this.selMark0.percentage,
 			i = this.selMark1.percentage;
 		return {
 			startPercentage: e,
 			startPosition: e * t,
 			endPercentage: i,
 	...
 	enableInteraction: function () {
 		this.drawer.interact = !0
 	},
 	disableInteraction: function () {
 		this.drawer.interact = !1
 	},
 ...
 WaveSurfer.Mark = {
 	...
 	init: function (t) {
 		...
 		this.params = t, this.loopSelection = this.params.loopSelection, this.ac = t.audioContext || this.getAudioContext(), this.offlineAc = this.getOfflineAudioContext(this.ac.sampleRate), this.createVolumeNode(), this.createScriptNode(), this.setPlaybackRate(this.params.audioRate)
 	},
 	...
 	refreshBufferSource: function () {
 		this.clearSource(), this.source = this.ac.createBufferSource(), this.playBackrate && (this.source.playbackRate.value = this.playBackrate), this.buffer && (this.source.buffer = this.buffer), this.source.connect(this.gainNode)
 	},
 	setupLoop: function () {
 		this.lastLoop = 0, this.loopedAtStart = !1, this.loop && this.lastStart <= this.loopEnd && (this.loopedAtStart = !0, this.source.loop = !0, this.source.loopStart = this.loopStart, this.source.loopEnd = this.loopEnd)
 	},
 	setBuffer: function (t) {
 		this.clearSource(), this.lastLoop = 0, this.lastPause = 0, this.lastStart = 0, this.startTime = 0, this.paused = !0, this.buffer = t
 	},
 	...
 	play: function (t, e) {
 		this.refreshBufferSource(), null == t && (t = this.getCurrentTime()), null == e && (e = this.getDuration()), t > e && (t = 0), this.lastStart = t, this.startTime = this.ac.currentTime, this.paused = !1, this.scheduledPause = e, this.loopSelection && this.setupLoop(), this.source.start ? this.source.start(0, t, e - t) : this.source.noteGrainOn(0, t, e - t), this.fireEvent("play")
 	},
 	pause: function () {
 		this.lastPause = this.loopIsActive() ? this.loopStart + (this.ac.currentTime - this.lastLoop) * this.playBackrate : this.lastStart + (this.ac.currentTime - this.startTime) * this.playBackrate, this.paused = !0, this.source && (this.source.stop ? this.source.stop(0) : this.source.noteOff(0), this.clearSource()), this.fireEvent("pause")
 	},
 	getPeaks: function (t) {
 		for (var e = this.buffer, i = e.length / t, r = ~~ (i / 10) || 1, s = e.numberOfChannels, n = new Float32Array(t), a = 0; s > a; a++)
 	...
 	getCurrentTime: function () {
 		return this.isPaused() ? this.lastPause : this.loopIsActive() ? this.loopStart + (this.ac.currentTime - this.lastLoop) * this.playBackrate : this.lastStart + (this.ac.currentTime - this.startTime) * this.playBackrate
 	},
 	audioContext: null,
 	...
 	updateSelection: function (t, e) {
 		if (!this.loopSelection) return !1;
 	...
 	init: function (t) {
 		...
 		this.params = t, this.pixelRatio = this.params.pixelRatio, this.loopSelection = this.params.loopSelection, this.width = 0, this.height = t.height * this.pixelRatio, this.containerWidth = this.container.clientWidth, this.interact = this.params.interact, this.lastPos = 0, this.createWrapper(), this.createElements()
 	},
 	...
 	setupWrapperEvents: function () {
 		...
 		this.wrapper.addEventListener("mousedown", function (i) {
 			t.interact && t.fireEvent("mousedown", e(i))
 	...
 	createElements: function () {
 		...
 		this.style(s, {
 			position: "absolute",
 			zIndex: 0
 		}), this.canvases = [t, i, r, s], this.waveCc = t.getContext("2d"), this.progressCc = i.getContext("2d"), this.progressWave = e, this.marksCc = r.getContext("2d"), this.selectionCc = s.getContext("2d")
 	},
 	updateWidth: function () {
 	...
 	drawSelection: function () {
 		...
 		var t = this.startPercent * this.width,
 			e = this.endPercent * this.width - t;
